...
function parseLocalCommand(text) {
    ...
    // E.g. "naya client balkrishna premnarayan add kar do, jiski monthly payment 25000 hai"
    ...
    const hasAddKw = cleanText.includes("add") || cleanText.includes("register") || cleanText.includes("banao") || cleanText.includes("bana do") || cleanText.includes("naya") || cleanText.includes("naye") || cleanText.includes("ऐड") || cleanText.includes("एड") || cleanText.includes("जोड़ो") || cleanText.includes("जोड़ो") || cleanText.includes("जोड़") || cleanText.includes("बना") || cleanText.includes("बनाओ");

    if (hasClientKw && hasAddKw) {
        let monthlyPay = 0;
        let textForName = text;
        
        // Match monthly payment keyword and amount
        const monthlyPayRegex = /(?:jiski|jiska|जिसकी|जिसका|की|का)?\s*(?:monthly\s+payment|monthly\s+pay|payment|salary|receivable|मंथली\s+पेमेंट|मंथली|मासिक\s+भुगतान|मासिक\s+पेमेंट|मासिक|प्रति\s+माह|पेमेंट|salary)\s*(?:hai|है)?\s*(?:₹\s*|rs\.?\s*)?(\d{3,8})(?:\s*(?:hai|है))?/i;
        const monthlyPayMatch = cleanText.match(monthlyPayRegex);
        if (monthlyPayMatch) {
            monthlyPay = parseInt(monthlyPayMatch[1]);
            const matchIndex = cleanText.indexOf(monthlyPayMatch[0]);
            if (matchIndex !== -1) {
                textForName = text.substring(0, matchIndex) + text.substring(matchIndex + monthlyPayMatch[0].length);
            }
        }

        let name = "";
        const cleanTextForName = textForName.toLowerCase().trim();
        ...
        if (cleanTextForName.includes("नाम से") || cleanTextForName.includes("name se")) {
            const separator = cleanTextForName.includes("के नाम से") ? "के नाम से" : (cleanTextForName.includes("name se") ? "name se" : "नाम से");
            const parts = textForName.split(new RegExp(separator, "i"));
            if (parts[0]) {
        ...
        if (!name) {
            ...
            for (const ind of nameIndicators) {
                if (cleanTextForName.includes(ind)) {
                    const parts = textForName.split(new RegExp(ind, "i"));
                    if (parts[1]) {
    ...
    if (isIncome) {
        ...
        let category = "Others";
        const dynamicCats = Object.keys(state.categoriesConfig);
        ...
        if (matchedCat) {
            ...
            if (!dynamicCats.includes(category)) {
                category = dynamicCats[0] || "Others";
            }
        }
...
async function executeAgentAction(actionObj) {
    ...
    if (actionObj.action === "addClient") {
        ...
        const monthlyPay = Number(actionObj.data.monthlyPay) || 0;
        const clientObj = {
            id: "c_" + Date.now(),
            name: clientName,
            monthlyPay: monthlyPay,
        ...
        if (monthlyPay > 0) {
            return `जय हरी! नया क्लाइंट "${clientName}" (मासिक भुगतान: ₹${monthlyPay.toLocaleString('en-IN')}) सफलतापूर्वक जोड़ दिया गया है।`;
        } else {
...
You must return JSON in this exact structure:
{
  ...
  "data": {
    "name": "extracted client name (transliterated to English/Latin characters)",
    "monthlyPay": number,
    "description": "expense details (e.g. travelling expenses, office tea)",
...
function cleanClientName(rawName) {
    // Remove punctuation first to avoid things like "do," not matching "do"
